...
float Drive::get_left_position_in(){
  return( LeftDrive.position(deg)*drive_in_to_deg_ratio );
}

float Drive::get_right_position_in(){
  return( RightDrive.position(deg)*drive_in_to_deg_ratio );
}
...
void Drive::drive_to_point(float X_position, float Y_position, float drive_max_voltage, float heading_max_voltage, float drive_settle_error, float drive_settle_time, float drive_timeout, float drive_kp, float drive_ki, float drive_kd, float drive_starti, float heading_kp, float heading_ki, float heading_kd, float heading_starti){
  ...
  while(drivePID.is_settled() == false){
    ...
    Brain.Screen.printAt(50, 150, "Heading output: %f", heading_output);
...
//Driver function for arcade control
void Drive::control_arcade(){
  float throttle = deadband(master.Axis3.value(), 5);
  float turn = deadband(master.Axis1.value(), 5);
  LeftDrive.spin(fwd, to_volt(throttle+turn), volt);
  RightDrive.spin(fwd, to_volt(throttle-turn), volt);
}
...
void Drive::joy_thresh_opcontrol(double l_stick, double r_stick) {
...
void Drive::arcadeDrive() {
  ...
  fwd_stick = left_curve_function(master.Axis3.position());
...
void Drive::singleStick() {
  double fwd_stick, turn_stick;
  // Put the joysticks through the curve function
  fwd_stick = left_curve_function(master.Axis3.position());
  turn_stick = right_curve_function(master.Axis4.position());
